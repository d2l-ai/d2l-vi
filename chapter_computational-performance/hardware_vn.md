...
Không phải tất cả các đơn vị đều được tạo ra như nhau.
Một vài đơn vị được chuyên môn hoá cho các lệnh về số nguyên trong khi đó một số khác được tối ưu hoá cho việc tính toán dấu chấm động.
...
Chẳng hạn, chúng ta có thể nhân ha số và cộng chúng với số thứ ba, cách này còn được biết đến như phép nhân-cộng hợp nhất (fused multiply-add).
[OpenVino](https://01.org/openvinotoolkit) của Intel sử dụng những điều này để đạt được thông lượng đáng nể cho học sâu ttreen CPUs máy chủ.
Tuy nhiên, xin lưu ý rằng con số này hoàn toàn không đáng kể so với khả năng của GPUs.
