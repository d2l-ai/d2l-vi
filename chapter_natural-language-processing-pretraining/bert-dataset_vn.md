...
Để tiền huấn luyện mô hình BERT như thực hiện trong :numref:`sec_bert`, ta cần sinh tập dữ liệu ở định dạng lý tưởng để thuận tiện cho hai tác vụ tiền huấn luyện: mô hình hóa ngôn ngữ có mặt nạ và dự đoán câu kế.
Một mặt, mô hình BERT gốc được tiền huấn luyện trên kho ngữ liệu được ghép lại từ bộ BookCorpus khổng lồ và Wikipedia Tiếng Anh (xem :numref:`subsec_bert_pretraining_tasks`), khiến việc thực hành là khó khăn đối với hầu hết bạn đọc cuốn sách này.
...
Do đó, tiền huấn luyện BERT trên một tập dữ liệu tùy chỉnh đang ngày càng trở nên phổ biến hơn.
Để thuận tiện việc minh họa cho tiền huấn luyện BERT, ta sử dụng một kho ngữ liệu nhỏ hơn là WikiText-2 :cite:`Merity.Xiong.Bradbury.ea.2016`.
...
Ở phần này, ta sẽ bắt đầu lập trình các hàm trợ giúp cho các hai tác vụ tiền huấn luyện BERT:
dự đoán câu kế và mô hình hóa ngôn ngữ có mặt nạ. 
...
Hàm sau đây sinh các mẫu huấn luyện cho việc dự đoán câu kế từ `paragraph` đầu vào thông qua hàm `_get_next_sentence`.
...
và `num_mlm_preds` chỉ định số lượng các dự đoán (nhớ lại 15% các token ngẫu nhiên để dự đoán). 
...
Cuối cùng, hàm này trả về những token đầu vào sau khi thực hiện thay thế,
