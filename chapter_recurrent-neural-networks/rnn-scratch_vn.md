...
1. Các phương pháp lấy mẫu khác nhau cho dữ liệu tuần tự (lấy mẫu ngẫu nhiên và phân tách tuần tự) sẽ dẫn đến sự khác biệt trong việc khởi tạo các trạng thái ẩn. <!--chỗ này là `random sampling` nhỉ? -->
...
Vì mẫu thứ $i^\mathrm{th}$ trong minibatch tiếp theo liền kề với mẫu thứ $i^\mathrm{th}$ trong minibatch hiện tại nên minibatch tiếp theo có thể sử dụng trực tiếp trạng thái ẩn hiện tại, ta chỉ tách gradient để  tính toán gradient cho mỗi minibatch.
...
Giống như hàm `train_epoch_ch3` trong :numref:`sec_softmax_scratch`, ta sử dụng thực thể `updater` để tổng quát hoá cả trường hợp lập trình súc tích với Gluon và lập trình từ đầu.
...
3. Thay đổi hàm dự đoán bằng việc lấy mẫu thay vì chọn ký tự tiếp theo là ký tự có khả năng cao nhất.
    * Điều gì sẽ xảy ra?
    * Điều chỉnh mô hình để ưu tiên các đầu ra có khả năng cao hơn, ví dụ, bằng cách lấy mẫu sử dụng $q(w_t \mid w_{t-1}, \ldots, w_1) \propto p^\alpha(w_t \mid w_{t-1}, \ldots, w_1)$ với $\alpha > 1$.
