...
Khác hoàn toàn các đoạn gõ phím ngẫu nhiên, văn bản được sinh ra từ mô hình này giống ngôn ngữ tự nhiên, giống tiếng Anh chẳng hạn.
...
Một cách khác ít chính xác hơn là đếm tất cả các lần xuất hiện của từ ”thống”, và chia nó cho tổng số từ trong kho dữ liệu văn bản. <!-- hm câu này phải là câu "Ví dụ, ..." ở trên thì mới hợp logic câu trước nữa ở L98 nhỉ? -->
...
Không may, việc ước tính xác suất của một cặp từ sẽ khó khăn hơn, bởi vì sự xuất hiện của cặp từ “thống kê” hiếm khi xảy ra hơn. <!-- Có lẽ nên lấy ví dụ khác, vì `thống kê` rất hay đi cùng nhau. Vì theo mình biết thì tiếng Việt là ngôn ngữ đơn âm tiết, 1 word chỉ có 1 âm tiết dù có mang ý nghĩa hay không (`thống`, `kê`), nên `Tokenization` sẽ có thêm 1 bước nữa là ghép `từ` (nếu cần) bằng cách xoá dấu cách giữa các cụm words: `thống kê` -> `thống_kê`, để đưa về giống tiếng Anh (1 word có thể có 1 hoặc nhiều âm tiết), sau đó mới xử lý tiếp. Nên đoạn này lấy ví dụ `Thống kê vui` thay cho `Statistic is fun` thì giải thích các khái niệm như ở câu trên sẽ hơi khó hiểu. Hoặc nên cân nhắc giữ ví dụ tiếng Anh, vì NLP còn phụ thuộc vào ngôn ngữ, nên nếu muốn dịch phần này dùng ví dụ tiếng Việt thì phải sửa theo.  -->
