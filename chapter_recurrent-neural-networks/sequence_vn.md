...
Ta sẽ đặt giá cổ phiếu là $x_t \geq 0$, tức là tại thời điểm $t \in \mathbb{N}$ ta thấy giá cổ phiếu bằng $x_t$.
...
Cụ thể là, số lượng đầu vào sẽ tăng cùng với lượng dữ liệu mà thu được và ta sẽ cần một phép tính xấp xỉ để làm cho tính toán này có thể thực hiện được.
...
1. Giả sử rằng việc sử dụng một chuỗi có khả năng là khá dài $x_{t-1}, \ldots, x_1$ là không thực sự cần thiết.
...
Điều này sẽ cho phép ta huấn luyện một mạng sâu như được đề cập ở phía trên.
Các mô hình như vậy được gọi là các mô hình *tự hồi quy*, vì chúng thực sự tự thực hiện hồi quy.
2. Một chiến lược khác, được biểu diễn trong :numref:`fig_sequence-model`, là giữ một số tóm tắt $h_t$ của các quan sát trong quá khứ, đồng thời cập nhật $h_t$ bên cạnh việc dự đoán $\hat{x}_t$.
Điều này dẫn đến việc các mô hình ước tính $x_t$ với $\hat{x}_t = p(x_t \mid x_{t-1}, h_{t})$ và cập nhật với biểu mẫu $h_t = g(h_{t-1}, x_{t-1})$.
Vì $h_t$ không bao giờ được quan sát, các mô hình này còn được gọi là các *mô hình tự hồi quy ẩn*.
