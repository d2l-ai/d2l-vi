...
Ở chương trước, ta đã giới thiệu hồi quy softmax (:numref:`sec_softmax`), 
cách lập trình thuật toán từ đầu (:numref:`sec_softmax_scratch`) hoặc sử dụng gluon (:numref:`sec_softmax_gluon`) 
và cách huấn luyện các mô hình phân loại để nhận dạng 10 loại quần áo từ bộ ảnh có độ phân giải thấp.
Trong suốt quá trình, ta đã học được cách xử lý dữ liệu, ép các đầu ra vào một phân phối xác xuất hợp lệ (thông qua `softmax`), áp dụng một hàm mất mát phù hợp và tối thiểu hóa nó ứng với các tham số của mô hình.
Bây giờ khi đã làm chủ được các cơ chế này trong bối cảnh những mô hình tuyến tính đơn giản,
ta có thể bắt đầu khám phá các mạng nơ-ron sâu, một lớp đa dạng các mô hình mà cuốn sách này chủ yếu quan tâm tới.
...
Để bắt đầu, hãy nhớ lại kiến trúc mô hình tương ứng với ví dụ về hồi quy softmax, minh họa ở :numref:`fig_singlelayer` bên dưới.
Mô hình này kết nối trực tiếp đầu vào tới đầu ra thông qua một biến đổi tuyến tính.
...
Nếu các nhãn thực sự liên kết tới dữ liệu đầu vào bởi một hàm tuyến tính thì phương pháp này là đủ.
Tuy nhiên tính chất tuyến tính là một *giả định mạnh*.
...
Ví dụ, tính chất tuyến tính bao hàm giả định *yếu hơn* của *tính đơn điệu*:
bất kỳ sự gia tăng nào của đặc trưng phải luôn dẫn đến hoặc là sự tăng thêm của đầu ra của mô hình (nếu trọng số tương ứng là dương), hoặc là sự giảm đi của đầu ra của mô hình (nếu trọng số tương ứng là âm).
Đôi khi điều này là hợp lý.
Ví dụ, khi ta cố gắng dự đoán liệu một người sẽ trả lại một khoản vay,
ta có thể hình dung một cách có lý rằng nếu giữ tất cả những yếu tố khác như nhau, một ứng viên với thu nhập cao sẽ luôn có khả năng hoàn lại lớn hơn so với một người có thu nhập thấp hơn.
Trong khi mối quan hệ này có thể là đơn điệu
thì việc tăng thu nhập từ $0 lên $50k có lẽ sẽ tương ứng với sự gia tăng lớn hơn trong khả năng trả nợ so với việc tăng từ $1M lên $1.05M.
Một cách để giải quyết vấn đề này là tiền xử lý dữ liệu để tính chất tuyến tính trở nên có vẻ hợp lý hơn, ví dụ, bằng cách sử dụng logarit của thu nhập như một đặc trưng.
